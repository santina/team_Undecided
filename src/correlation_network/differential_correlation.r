...
computeNetworkWeights <- function(genes, groups, weightsDir) {
  ...
  # groupPairNames <- groupPairs %>% names()
  # groupPairNames <- gsub("\\.", "_", groupPairNames)
...
# **** ARGUMENTS ****
runID_arg <- "FINAL_RUN_MACBOOK" # give a meaningful name to the data generated by this specific run
genes_arg <- read.table("gene_list_Eric.txt")[[1]] %>% as.character()
networkWeights_arg <- "network_weights"
...
networkWeights <- computeNetworkWeights(genes, groups, networkWeights_arg)
