...
chMatrix <- data.frame(do.call('rbind', strsplit(as.character(conHigh$gene_pair),'.',fixed=TRUE)))
chMatrix$weight <- conHigh$abs_correlation_change

clMatrix <- data.frame(do.call('rbind', strsplit(as.character(conLow$gene_pair),'.',fixed=TRUE)))
clMatrix$weight <- conLow$abs_correlation_change

hlMatrix <- data.frame(do.call('rbind', strsplit(as.character(highLow$gene_pair),'.',fixed=TRUE)))
hlMatrix$weight <- highLow$abs_correlation_change

write.table(chMatrix, file = "chWeight.tsv")
write.table(clMatrix, file = "clWeight.tsv")
write.table(hlMatrix, file = "hlWeight.tsv")


chFilter <- conHigh[conHigh$fdr<=0.05,]$gene_pair %>% strsplit(".", fixed= TRUE) %>% unlist() %>% unique()

clFilter <- conLow[conLow$fdr<=0.05,]$gene_pair %>% strsplit(".", fixed= TRUE) %>% unlist() %>% unique()

hlFilter <- highLow[highLow$fdr<=0.05,]$gene_pair %>% strsplit(".", fixed= TRUE) %>% unlist() %>% unique()
``` 
