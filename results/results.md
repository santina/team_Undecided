...
As a general introduction to the data, we have two main datasets, both downloaded from GEO with accession number [GSE85568](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE85568).  They are both stored in the raw_data subfolder.  The first one consists of normalized RNA-seq counts from protein-mapping genes annotated in ENSEMBL consisting of more than 5 reads in two patients, leaving 16,535 gene entries from 85 patients (57 asthmatics, 28 controls).  There were 71 asthmatics and 41 controls in the original.  Only 85 patients had both RNA-seq and methylation data available, and only 81 had all clinical data available.  The methylation dataset included 327,271 probes from 85 patients, and had been corrected for probe bias (using SWAN), filtered for uniquely mapping probes and adjusted for the effect of CHIP using COMBAT. 
...
After, we performed differential expression analysis with the RNA-seq data using edgeR to extract a list of "interesting genes" (or genes with high differential expression between our three groups) based on FDR.  This step was vital, because performing differential network analysis is computationally heavy, so running it on our entire set of genes would be infeasible.  Using edgeR, we obtained a list of genes, ordered by FDR.  Setting a threshold of FDR <= 0.01 filtered our list down to 571, which we calculated would take around 30 hours to run for our differential network analysis step.  
...
*Output*: three lists (corresponding to control vs high, control vs low, and high vs low) of weights associated with each gene, depending on the significance of differential methylation between groups (the lower the FDR, the stronger the weight).  

Differential methylation analysis was performed using limma, and included gender, ethnicity, age and smoking status as covariates. To define a weight for the significance of differential methylation for each gene, the -log2(FDR) associated with each gene was recorded. More differentially methylated CpGs were found when comparing asthmatics to controls then between asthma subtypes, as the "weights" in the Th2-high to Th2-low comparison tended to be low (all weights < 3), compared to the Th2-high to control comparison, with two weights > 30, and compared to the Th2-low to control comparison, which had several weights > 13.  This indicates that biological changes between asthma subtypes are likely more subtle than changes between asthmatic and healthy patients. 

To generate a single methylation weight for each gene, only the most significant probe of all mapping to the same gene was taken as the weight for that gene.
...
### Control vs. Th2-Low: PACSIN1 vs. SPATS2 distribution
![pacsinvspats](https://github.com/STAT540-UBC/team_Undecided/blob/master/results/figures/figure6_distribution_control_low_pacsin1_spats2.png "PACSIN1 vs SPATS2")
