...
```{r}
model.matrix(~cluster+Gender+Age+current_smoker, metadata)
model.matrix(~cluster*Gender, metadata)
...
filt1 <- (topTags(lrtTEST, n= Inf))$table %>% rownames_to_column(var="Gene") %>% filter(FDR <= .01)

lrtTEST2 <- glmLRT(fitTEST, contrast=c(-1,0,1))
filt2 <- (topTags(lrtTEST2, n= Inf))$table %>% rownames_to_column(var="Gene") %>% filter(FDR <= .25)
filt2 %>% nrow()

lrtTEST3 <- glmLRT(fitTEST, contrast=c(0,1,-1))
filt3 <- (topTags(lrtTEST3, n= Inf))$table %>% rownames_to_column(var="Gene") %>% filter(FDR <= .01)



heyya <- union(filt1$Gene, filt2$Gene) %>% union(filt3$Gene)
heyya %>% length()

write.table(heyya, "gene_list_Eric.txt")


emmaGenes <- read.table("~/Downloads/meth_CtlvsTh2high_pval0.01_ensemblID.txt", col.names = 2)

emmaGenes2 <- read.table("~/Downloads/meth_CtlvsTh2low_pval0.01_ensemblID.txt", col.names = 2)

emmaGenes %>% nrow()
emmaGenes2 %>% nrow() 
intersect(emmaGenes2$X2, emmaGenes$X2) %>% length()
```
